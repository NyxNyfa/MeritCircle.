...
export class OnChainSettlementProvider implements SettlementProvider {
  ...
  async settleCycle(params: SettleCycleParams): Promise<SettlementResult> {
    ...
    try {
      ...
      const recipientAddress = getAddress(params.recipientWalletAddress);
      ...
      if (receipt.status === "reverted") {
        throw new Error(`Smart contract settlement method reverted on-chain. TxHash: ${txHash}`);
      }
      ...
      return {
        success: false,
        reason: `Penyelesaian on-chain gagal: ${err?.shortMessage || err?.message || "Kesalahan jaringan smart contract"}`,
      };
    }
  }
}
